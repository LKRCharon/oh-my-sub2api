...
const codexChartColor = '#14B8A6'

const getModelChartColor = (model: string, index: number): string => {
    ...
    datasets: [
      {
        ...
        backgroundColor: displayModelStats.value.map((model, index) => getModelChartColor(model.model, index)),
        borderWidth: 0
      }
    ]
  }
